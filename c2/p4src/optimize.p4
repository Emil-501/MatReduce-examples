...
control MyIngress(inout headers hdr,
    ...
    action nat_int_to_ext(bit<32> src_addr, bit<16> port_span) {
        ...
        hdr.tcp.srcPort = hdr.tcp.srcPort + port_span;
    }
    action nat_ext_to_int(bit<32> dst_addr, bit<16> port_span) {
        hdr.ipv4.dstAddr = dst_addr;
        hdr.tcp.dstPort = hdr.tcp.dstPort - port_span;
    }
    table ecmp_group {
        key = {
    ...
    table composed_t {
        ...
        key = {
            hdr.ethernet.srcAddr         : ternary; // used for ipv4_sg
            hdr.ipv4.srcAddr             : ternary; // used for ipv4_sg, nat
            hdr.ipv4.dstAddr             : ternary; // used for nat
            hdr.tcp.srcPort              : ternary; // used for nat
            hdr.tcp.dstPort              : ternary; // used for nat
        }
    }
